...
new Promise(inputANumberExecutor)
    .then((resolvedValue) => mustBeGreaterThanZero(resolvedValue)) // is the number greater than zero?
    .then((resolvedValue) => isEvenNumber(resolvedValue)) // is the number even?
    .then((resolvedValue) => logSuccessFailure(resolvedValue)) // log the success/failure of the user's input
