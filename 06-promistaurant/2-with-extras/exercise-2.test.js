...
const preparedExtras = Promise.all(
    [extras.springOnion, extras.broccoli, extras.roastedPeanuts].map((extra) =>
        prepareExtra(extra),
    ),
);
...
const geraltsMeal = Promise.all([preparedExtras, mealWithoutExtras])
    .then(([preparedExtras, meal]) => addPreparedExtras(meal, preparedExtras))
    .then((meal) => bag(meal));
        ...
        describe("Geralt's meal", () => {
            ...
            describe('the meal should have 3 extras', () => {
                ...
                it('includes: spring onion', () => {
                    expect(theMeal.extras.includes(extras.springOnion)).toEqual(
                        true,
                    );
                });
                it('includes: broccoli', () => {
                    expect(theMeal.extras.includes(extras.broccoli)).toEqual(
                        true,
                    );
                });
                it('includes: roasted peanuts', () => {
                    expect(
                        theMeal.extras.includes(extras.roastedPeanuts),
                    ).toEqual(true);
                });
            });
        });
